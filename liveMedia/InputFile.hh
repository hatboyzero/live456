...
LIVEMEDIA_DLL_LINK FILE* OpenInputFile(UsageEnvironment& env, char const* fileName);

LIVEMEDIA_DLL_LINK void CloseInputFile(FILE* fid);

LIVEMEDIA_DLL_LINK u_int64_t GetFileSize(char const* fileName, FILE* fid);
    // 0 means zero-length, unbounded, or unknown

LIVEMEDIA_DLL_LINK int64_t SeekFile64(FILE *fid, int64_t offset, int whence);
    // A platform-independent routine for seeking within (possibly) large files

LIVEMEDIA_DLL_LINK int64_t TellFile64(FILE *fid);
    // A platform-independent routine for reporting the position within
    // (possibly) large files

LIVEMEDIA_DLL_LINK Boolean FileIsSeekable(FILE *fid);
